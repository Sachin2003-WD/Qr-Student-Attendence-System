        ...
        {batchesList.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {batchesList.map((b, idx) => {
              ...
              return (
                <Card key={b.id || idx} className="border border-border/60 hover:border-primary/50 transition-all shadow-xs overflow-hidden bg-card">
                  <CardHeader className="bg-muted/30 pb-3 border-b border-border/40">
                    ...
                    <CardTitle className="text-base font-bold text-foreground line-clamp-1">{b.subjectName || "Grooming & Skills"}</CardTitle>
                    <CardDescription className="text-xs flex items-center gap-1 text-muted-foreground">
                      <Building2 className="h-3 w-3 text-muted-foreground" /> {b.branch || "Rajajinagar Jspiders"}
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="pt-4 space-y-3">
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="space-y-0.5">
                        ...
                        <p className="font-medium text-foreground">{b.startDate || "24-Jun-2026"}</p>
                      </div>
                      <div className="space-y-0.5">
                        <span className="text-muted-foreground flex items-center gap-1">
                          <Clock className="h-3 w-3" /> Class Timing
                        </span>
                        <p className="font-medium text-foreground font-mono">{b.classTiming || "04:45 PM"}</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs font-semibold text-foreground">{b.trainerName || "Laxman Ashok Handenavar"}</p>
                    </div>
                    ...
                    >
                      <History className="h-3.5 w-3.5 text-primary" /> View Student Attendance Roster
                    </Button>
                  </CardContent>
                </Card>
              );
