...
export const Route = createFileRoute("/admin-register")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Dedicated administrator registration form with security authorization." },
    ],
  }),
...
function AdminRegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.securityKey.trim().toUpperCase() !== "ADMIN2026") {
      toast.error("Invalid Admin Security Code! Only authorized institution administrators can register.");
      return;
    }
    ...
    <div className="min-h-screen grid lg:grid-cols-2 bg-background">
      ...
      <div className="relative hidden overflow-hidden bg-gradient-to-br from-indigo-700 via-purple-800 to-slate-900 p-10 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="max-w-md space-y-4">
          ...
          <h2 className="text-4xl font-extrabold leading-tight">Secure Administrator Registration</h2>
          <p className="text-indigo-100/90 text-sm leading-relaxed">
            Register institution coordinator accounts with strict security passcode verification to manage academic batches, logs & QR engines.
          </p>
        </div>
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4 text-left">
                <div className="space-y-2">
                  <Label htmlFor="ar-name" className="text-xs font-semibold">Administrator Full Name</Label>
                  <Input
                    id="ar-name"
                    required
                    value={formData.name}
                    onChange={(e) => updateField("name", e.target.value)}
                    placeholder="e.g. Dr. Admin Smith"
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    <Label htmlFor="ar-email" className="text-xs font-semibold">Official Email Address</Label>
                    <Input
                      id="ar-email"
                      type="email"
                      required
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="ar-phone" className="text-xs font-semibold">Mobile Phone (10 digits)</Label>
                    <Input
                      id="ar-phone"
                      required
                      maxLength={10}
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <Label htmlFor="ar-sec" className="text-xs font-semibold flex items-center gap-1.5 text-indigo-600 dark:text-indigo-400">
                      <KeyRound className="h-3.5 w-3.5" /> Admin Security Key
                    </Label>
                    <span className="text-[10px] text-muted-foreground">Default Code: ADMIN2026</span>
                  </div>
                  <Input
                    id="ar-sec"
                    type="text"
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    Prevents students from registering as Admin. Default authorization key is <code className="font-mono text-indigo-600 font-bold">ADMIN2026</code>.
                  </p>
                </div>

                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    <Label htmlFor="ar-pass" className="text-xs font-semibold">Password</Label>
                    <Input
                      id="ar-pass"
                      type="password"
                      required
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="ar-cpass" className="text-xs font-semibold">Confirm Password</Label>
                    <Input
                      id="ar-cpass"
                      type="password"
                      required
